...
1. 修改脚本开头两个参数：
   - `PhoneIps`：上一步查到的手机 IP。只有一台就保留 `@("10.94.18.52")`；多台写成 `@("10.94.18.52", "10.94.18.53")`
...
3. 双击快捷方式：
   - 所有手机会显示在同一个面板里，每台一行
   - 每行显示 `手机名称 OK` / `手机名称 NO`，默认会读取手机型号名
   - 点 `Rename` 可以改成自己容易分辨的名字，名称保存在脚本同目录的 `devices.json`
   - 点 `PC On` / `PC Off` 控制这台手机是否推流到电脑；`PC Off` 时手机自己播放
   - 点对应行的 `Play/Pause` 切换那台手机的播放状态
   - 拖动状态文字可移动整个面板
   - 把面板拖到屏幕上边缘会自动收起成一条细线，鼠标移上去再展开
   - 点右上角 `X` 关闭面板并停止所有手机的音频传输

多台手机的音频会同时混合到电脑默认播放设备，可以同时从同一副耳机里听到。
脚本默认把音频缓冲降到 `20ms` 并用 `raw` 音频编码以降低延迟；如果出现卡顿或爆音，可以把 `AudioBufferMs` 调回 `50`，或把 `AudioCodec` 改回 `opus`。

## 播放优先级

在面板上点某台手机的 `Play/Pause` 时，会先自动暂停其他手机，再切换这台，所以“最后一次在面板上操作”的那台会优先播放。

注意：如果直接用手机本机按钮开始播放，面板无法感知，不会自动暂停其他手机。
