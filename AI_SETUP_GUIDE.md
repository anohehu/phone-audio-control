...
6. 验证
   ...
   - 点 `PC On` / `PC Off` 控制该手机是否推流到电脑；`PC Off` 时手机自己播放
...
- 延迟：脚本默认 `AudioBufferMs=20` 和 `AudioCodec=raw`；出现卡顿或爆音时把 `AudioBufferMs` 改回 `50` 或把 `AudioCodec` 改回 `opus`，想进一步降延迟可改用 USB 连接
